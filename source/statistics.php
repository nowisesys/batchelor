...
function print_links_section($data, $subdirs, $child, $parent)
{
        ...
        if ($child) {
                ...
                foreach ($subdirs as $subdir) {
                        printf("<a href=\"statistics.php?%s\">%s %s </a>", request_params(array("$child" => $subdir)), $delim, subsect_date_string($child, $subdir));
                        if (strlen($delim) == 0) {
                                $delim = "|";
                        }
                }
...
function print_state_section($data, $statdir, $subsect)
{
        ...
        printf("<p>Totally %d jobs was <b>finished</b> (%d of them with <b>warnings</b>). ", $data['state']['warning'] + $data['state']['success'], $data['state']['warning']);
        printf("%d ended with <b>errors</b> and %d has <b>crashed</b>.", $data['state']['error'], $data['state']['crashed']);
        printf("</p>\n");
...
function stat_show_root($statdir, $hostid)
{
        ...
        if ($data) {
                ...
                if ($_REQUEST['stat'] == "load") {
                        print_sysload_section($data, $statdir, array("sysload_total", "sysload_weekly", "sysload_hourly"), $subsect);
                }
        }
}
...
function print_body()
{
        ...
        if (!file_exists($statdir)) {
                print_message_box("info", sprintf("No statistics has been generated yet. Contact %s to have this fixed.", CONTACT_STRING));
                return;
        }
