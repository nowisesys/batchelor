...
function check_params(&$request, &$entry)
{
        if (isset($entry['params'])) {
                ...
                if (count($request['params']) < count($entry['params'])) {
                        send_error(WS_ERROR_MISSING_PARAMETER, sprintf("Expected %d parameters, but got %d.", count($entry['params']), count($request['params'])), true);
                        return false;
                }
                if (count($request['params']) > count($entry['params'])) {
...
function send_response($request)
{
        ...
        switch ($entry['name']) {
                ...
                case "suspend":
                        if (check_params($request, $entry)) {
                                if (($status = ws_suspend(get_request_param($request, 0), get_request_param($request, 1)))) {
                                        send_params_start();
                ...
                case "resume":
                        if (check_params($request, $entry)) {
                                if (($status = ws_resume(get_request_param($request, 0), get_request_param($request, 1)))) {
                                        send_params_start();
                                        send_boolean_response($status);
                                        send_params_end();
                                }
                ...
                case "dequeue":
                        if (check_params($request, $entry)) {
                                if (($status = ws_dequeue(get_request_param($request, 0), get_request_param($request, 1)))) {
                                        send_params_start();
                                        send_boolean_response($status);
                                        send_params_end();
                                }
                        ...
                        if (isset($request['params'])) {
                                if (isset($request['params'][0])) {
                                        if (isset($request['params'][1])) {
                                                if (($status = ws_queue($result, get_request_param($request, 0), get_request_param($request, 1)))) {
                                                        send_params_start();
                                                        send_queue_response($result);
                                                        send_params_end();
                                                }
                ...
                case "readdir":
                        if (check_params($request, $entry)) {
                                if (($status = ws_readdir(get_request_param($request, 0), get_request_param($request, 1), $result))) {
                                        send_params_start();
                                        send_readdir_response($result);
                                        send_params_end();
                                }
                ...
                case "fopen":
                        if (check_params($request, $entry)) {
                                if (($status = ws_fopen(get_request_param($request, 0), get_request_param($request, 1), get_request_param($request, 2), $result))) {
                                        send_params_start();
                ...
                case "stat":
                        if (check_params($request, $entry)) {
                                if (($status = ws_stat(get_request_param($request, 0), get_request_param($request, 1), $result))) {
                                        send_params_start();
