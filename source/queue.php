...
function show_jobs_table_icons(&$jobs)
{
        ...
        foreach ($jobs as $jobdir => $job) {
                ...
                switch ($job['state']) {
                        case "pending":
                                $title = sprintf("queued %s, \nwaiting in queue", format_timestamp($job['queued']));
                                break;
                        case "running":
                                $title = sprintf("started %s, \nstill running", format_timestamp($job['started']));
                                break;
                        case "finished":
                                $title = sprintf("started %s, \nfinished %s", format_timestamp($job['started']), format_timestamp($job['finished']));
                                break;
                        case "warning":
                                $title = sprintf("started %s, \nfinished (with warnings) %s", format_timestamp($job['started']), format_timestamp($job['stderr']));
                                break;
                        case "error":
                                $title = sprintf("started %s, \nfinished (with errors) %s", format_timestamp($job['started']), format_timestamp($job['stderr']));
                                break;
                        case "crashed":
                                $title = sprintf("started %s, \njob has crashed (not running)", format_timestamp(isset($job['started']) ? $job['started'] : 0));
                                break;
                }
                ...
                // Job column
                // 
                printf("<td nowrap><a href=\"details.php?jobid=%s&result=%s\" target=\"_blank\" title=\"%s\">Job %s</a></td>", $job['jobid'], $jobdir, $title, $job['jobid']);
                ...
                if (ENABLE_JOB_CONTROL != "off" && $job['state'] == "running") {
                        if (ENABLE_JOB_CONTROL == "simple") {
                                printf("<td nowrap><a href=\"jobcontrol.php?jobid=%s&result=%s&sort=%s&filter=%s&signal=%s\" title=\"send job the %s signal\"><img src=\"icons/nuvola/delete.png\" alt=\"signal\"></a></td></tr>", $job['jobid'], $jobdir, $_REQUEST['sort'], $_REQUEST['filter'], JOB_CONTROL_ACTION, JOB_CONTROL_ACTION);
                        } else {
                                global $signals;
        ...
        foreach ($jobs as $jobdir => $job) {
                ...
                switch ($job['state']) {
                        ...
                        case "crashed":
                                ...
                                $title = sprintf("started %s, \njob has crashed (not running)", format_timestamp($job['started']));
                                break;
                }
                ...
                // Job column
                // 
                printf("<td nowrap><a href=\"details.php?jobid=%s&result=%s\" target=\"_blank\" title=\"%s\">Job %d</a></td>", $job['jobid'], $jobdir, $title, $job['jobid']);
                ...
                $links = array();
                if (SHOW_JOB_DELETE_LINK && $job['state'] != "running") {
                        array_push($links, sprintf("<a href=\"delete.php?jobid=%s&result=%s&sort=%s&filter=%s\">delete</a>", $job['jobid'], $jobdir, $_REQUEST['sort'], $_REQUEST['filter']));
                }
                ...
                if (PAGE_REFRESH_RATE > 0) {
                        ...
                        foreach ($jobs as $job) {
                                if ($job['state'] == "pending" || $job['state'] == "running") {
                                        ...
                                        if (isset($_REQUEST['js']) && $_REQUEST['js'] == "on" && WS_ENABLE_HTTP && extension_loaded("json")) {
...
function check_request_param($name, $accepted)
{
        if (!in_array($_REQUEST[$name], $accepted)) {
                error_exit(sprintf("Invalid value '%s' for request parameter '%s'", $_REQUEST[$name], $name));
        }
}
...
if (isset($_FILES['file']['name']) || isset($_REQUEST['data'])) {
        ...
        // 
        header(sprintf("Location: queue.php?queued=yes&jobs=%d&proc=%s", count($jobs), isset($_REQUEST['proc']) ? $_REQUEST['proc'] : FORM_SUBMIT_TYPE));
}
